...
4. **High Level Requirements** — Extract functional requirements from the RFE. Add priority markers: [P0] must have (blocks ship), [P1] should have (important, not blocking), [P2] nice to have (defer if effort exceeds estimate), [Deferred] explicitly out of scope for this release. If the RFE already has user stories, preserve them with priority markers added. When an HLR is already implemented, append status: `— *DONE* (PR #NNN)` — this prevents downstream consumers from planning work that's already complete.
