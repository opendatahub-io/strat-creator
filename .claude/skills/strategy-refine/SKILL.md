...
In local mode only (`workflow: local`), save a version snapshot and generate a diff after writing the strategy:

```bash
python3 scripts/strategy_history.py save local/strat-tasks/<filename>.md
```

This saves the current file as the next version (v1, v2, ...) in `local/strat-history/<strat_id>/`. If a prior version exists, it generates a word-level HTML diff and updates the `latest_diff` frontmatter field.
