...
2. **Affected Components** — Which platform components are touched? Reference actual component names from the architecture context. Exception: upstream projects the inventory does not list as their own component. vLLM-Omni (`vllm-project/vllm-omni`) is the running case; the architecture context has no `vllm-omni` component doc, only overlay mentions. List it as its own row, `**vLLM-Omni** (<team>)`, with the repo name. Never fold it into a vLLM row. See `${CLAUDE_SKILL_DIR}/upstream-project-boundaries.md`.
...
- **Name the upstream repository, not the product family.** vLLM (`vllm-project/vllm`) and vLLM-Omni (`vllm-project/vllm-omni`) are separate repos with separate maintainers and PR numbering. Before writing Technical Approach or Dependencies for anything touching speech, audio or image or video generation, realtime, duplex, or "omni", read `${CLAUDE_SKILL_DIR}/upstream-project-boundaries.md` and pin each capability to the repo that owns it. If the source RFE attributes a capability to the wrong repo, say so in Open Questions and use the correct repo in the strategy; do not edit the Business Need section. Every PR or issue reference in Dependencies carries repo, state, author, and date (`vllm-project/vllm-omni PR #NNNN, open, <author>, last commit <date>`). A bare number is not a citation. Mark anything you could not verify from code `NEEDS VERIFICATION`.
