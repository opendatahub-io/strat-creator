...
STAFF_INPUT_ATTACHMENT_TEMPLATE = "{strat_key}-staff-input.md"
...
def pull_strategy(server, user, token, strat_key, local_dir="local"):
    ...
    # Check for staff input attachment
    si_att_filename = STAFF_INPUT_ATTACHMENT_TEMPLATE.format(strat_key=strat_key)
    staff_input_attachment = None
    for att in attachments:
        if att.get("filename") == si_att_filename:
            staff_input_attachment = att

    if staff_input_attachment:
        with tempfile.NamedTemporaryFile(
                mode="wb", suffix=".md", delete=False) as tmp:
            si_tmp_path = tmp.name
        try:
            download_attachment(server, user, token,
                                staff_input_attachment["content"], si_tmp_path)
            with open(si_tmp_path, "r", encoding="utf-8") as f:
                full_staff_input = f.read().strip()
        finally:
            os.unlink(si_tmp_path)

        if re.search(STAFF_INPUT_PATTERN, description_md):
            description_md = re.sub(
                STAFF_INPUT_PATTERN + r'.*?(?=\n## (?!#)|$)',
                lambda _: full_staff_input + "\n",
                description_md,
                flags=re.DOTALL,
            )
        else:
        ...
        print(f"  Staff Input loaded from attachment: {si_att_filename}")
