...
STRAT_KEY_RE = re.compile(r'^RHAISTRAT-\d+$')
...
def pull_strategy(server, user, token, strat_key, local_dir="local"):
    ...
    """
    if not STRAT_KEY_RE.match(strat_key):
        print(f"Error: invalid strat_key format: {strat_key!r}", file=sys.stderr)
        sys.exit(1)
...
def main():
    ...
    if not STRAT_KEY_RE.match(args.strat_key):
