...
def _upload_staff_input_attachment(jira, strat_key, staff_input_content):
    """Upload a staff input markdown file as an attachment."""
    with tempfile.NamedTemporaryFile(
            mode="w", suffix=".md", delete=False, encoding="utf-8") as f:
    ...
    try:
        add_attachment(jira.url, "admin", "admin", strat_key,
                       tmp_path, filename=f"{strat_key}-staff-input.md")
    finally:
        os.unlink(tmp_path)


class TestPullWithStaffInputAttachment:

    def test_pulls_staff_input_from_attachment(self, jira, tmp_path):
        """Tier 2 scenario: strategy inline, Staff Input in attachment."""
        stub_desc = (
            "## Business Need (from RFE)\n\n"
            "Users need GPU sharing.\n\n"
            f"{STRATEGY_HEADING}\n\n"
            ...
            "`RHAISTRAT-2070-staff-input.md`.\n"
        )
        jira.create("RHAISTRAT-2070", "Staff input attachment test",
                     stub_desc, labels=["strat-creator-rubric-pass"])
        jira.create("RHAIRFE-3070", "GPU sharing RFE",
                     "Users need cost-efficient GPU sharing.")
        jira.request("POST", "/rest/api/3/issueLink", {
            "type": {"name": "Cloners"},
            "inwardIssue": {"key": "RHAISTRAT-2070"},
            "outwardIssue": {"key": "RHAIRFE-3070"},
        })
        ...
        _upload_staff_input_attachment(jira, "RHAISTRAT-2070", staff_input)

        local_dir = tmp_path / "local"
        result = _run(jira, "RHAISTRAT-2070", local_dir)
        assert result.returncode == 0, f"stderr: {result.stderr}"
        assert "staff input" in result.stdout.lower()

        strat_path = local_dir / "strat-tasks" / "RHAISTRAT-2070.md"
        content = strat_path.read_text()
        assert "Use Argo Workflows, not Tekton" in content
        assert "Use MIG profiles" in content
        assert "stored as attachment" not in content

    def test_pulls_both_strategy_and_staff_input_attachments(self, jira, tmp_path):
        """Tier 3 scenario: both strategy and Staff Input in attachments."""
        stub_desc = (
            "## Business Need (from RFE)\n\n"
            "Users need GPU sharing.\n\n"
            f"{STRATEGY_HEADING}\n\n"
            "> **Note:** The full strategy exceeds Jira's description size "
            "limit and is stored as an attachment: "
            "`RHAISTRAT-2071-strategy.md`.\n\n"
            "### TL;DR\n\n"
            "Enable GPU sharing via MIG.\n\n"
            f"{STAFF_INPUT_HEADING}\n\n"
            "> **Note:** Staff Engineer / SME Input is stored as attachment: "
            "`RHAISTRAT-2071-staff-input.md`.\n"
        )
        jira.create("RHAISTRAT-2071", "Both attachments test",
                     stub_desc, labels=["strat-creator-rubric-pass"])
        jira.create("RHAIRFE-3071", "GPU sharing RFE",
                     "Users need GPU sharing across tenants.")
        jira.request("POST", "/rest/api/3/issueLink", {
            "type": {"name": "Cloners"},
            "inwardIssue": {"key": "RHAISTRAT-2071"},
            "outwardIssue": {"key": "RHAIRFE-3071"},
        })
        ...
        _upload_strategy_attachment(jira, "RHAISTRAT-2071", full_strategy)

        staff_input = (
            f"{STAFF_INPUT_HEADING}\n\n"
            "Effort estimate should be L, not M.\n"
        )
        _upload_staff_input_attachment(jira, "RHAISTRAT-2071", staff_input)

        local_dir = tmp_path / "local"
        result = _run(jira, "RHAISTRAT-2071", local_dir)
        assert result.returncode == 0, f"stderr: {result.stderr}"

        strat_path = local_dir / "strat-tasks" / "RHAISTRAT-2071.md"
        content = strat_path.read_text()
        assert "NVIDIA device plugin with MIG profiles" in content
        assert "Effort estimate should be L, not M" in content
        assert "stored as attachment" not in content

    def test_backward_compat_inline_staff_input(self, jira, tmp_path):
        """Old-style issue with Staff Input inline (no attachment)."""
        desc = (
            "## Business Need (from RFE)\n\n"
            "Users need GPU sharing.\n\n"
            f"{STRATEGY_HEADING}\n\n"
        ...
        jira.create("RHAISTRAT-2072", "Inline staff input", desc,
                     labels=["strat-creator-rubric-pass"])
        jira.create("RHAIRFE-3072", "GPU RFE", "GPU sharing needed.")
        jira.request("POST", "/rest/api/3/issueLink", {
            "type": {"name": "Cloners"},
            "inwardIssue": {"key": "RHAISTRAT-2072"},
            "outwardIssue": {"key": "RHAIRFE-3072"},
        })

        local_dir = tmp_path / "local"
        result = _run(jira, "RHAISTRAT-2072", local_dir)
        assert result.returncode == 0, f"stderr: {result.stderr}"

        strat_path = local_dir / "strat-tasks" / "RHAISTRAT-2072.md"
        content = strat_path.read_text()
        assert "Use Argo Workflows, not Tekton" in content
        assert "Use MIG profiles" in content
